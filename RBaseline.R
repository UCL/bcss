...
input <- data.frame(rho=0.01, WCC1=0.5,WCC2=0.9,n=200, select=1,ytop=1.25,ybot=0.5)
...
plot2 <- ({ if ( (input$select=="1") ) { 
  ggplot(data.frame(x=c(0,1)),aes(x=x,colour=legends))+ggtitle(label=paste("size=",input$n," ICC=",input$rho," pi=",input$WCC1," ",input$WCC2),subtitle=paste("opt(x,y)=  (",Lopt_x,",",Lopt_y,")  (",Hopt_x,",",Hopt_y,")" ))+ 
